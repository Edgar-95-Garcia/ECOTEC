...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_SESSION['admin_ecotec'])) {
    $flag = true;
    if ($_POST["modificar_profesor"] == "ACTUALIZAR") {
        ...
        $id_profesor = htmlentities($_POST["id_profesor"]);
        ...
        if ($flag == true) {
            ...
            $a = $modificar->updateProfesor($nombres,$apellido_paterno,$apellido_materno,$telcontact,$correo,$contraseña,$status,$level,$id_profesor);
            if ($a == 1) {
                echo '<script type="text/javascript">alert("Datos de profesor modificados");</script>';
?>
                <script>
                    window.location.replace("admon_administrar_profesores.php");
                </script>
<?php
            } elseif ($a == 0) {
                echo '<script type="text/javascript">alert("Datos de profesor no modificados, por favor intente en unos minutos");</script>';
            }
        } else {
